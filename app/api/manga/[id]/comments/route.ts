  ...
  try {
    ...
    if (auth) {
      comments = await query<{
        ...
        user_id: string; username: string; avatar: string; user_liked: boolean
      }>(
        `SELECT c.id, c.content, c.likes_count, c.created_at,
                u.id as user_id, u.username, u.avatar,
                EXISTS(SELECT 1 FROM comment_likes cl WHERE cl.comment_id=c.id AND cl.user_id=$3) as user_liked
         FROM comments c
         JOIN users u ON u.id = c.user_id
         WHERE c.manga_id = $1
         ORDER BY c.created_at DESC
         LIMIT $2 OFFSET $4`,
        [id, limit, auth.userId, offset]
      )
    } else {
      comments = await query<{
        id: string; content: string; likes_count: number; created_at: string
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    if (content.trim().length > 1000) return apiError('Izoh 1000 ta belgidan oshmasligi kerak')
    ...
    const user = await queryOne<{ username: string; avatar: string }>(
      'SELECT username, avatar FROM users WHERE id=$1', [auth.userId]
    )
